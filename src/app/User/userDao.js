...
async function selectUserEmail(connection, email) {
  const selectUserEmailQuery = `
                SELECT email, nickname, status, userId, loginType
                FROM userTB 
                WHERE email = ?;
                `;
...
async function selectUserSub(connection, sub) {
  const selectUserEmailQuery = `
                SELECT jwt,refreshToken, sub, email, userTB.userId
                FROM userTB
                JOIN tokenTB on tokenTB.userId = userTB.userId
                WHERE sub = '${sub}';
                `;
  const [subRows] = await connection.query(selectUserEmailQuery, sub);
  return subRows;
}
...
async function retrieveUserStatus(connection,userId) {
  const retrieveUserStatusQuery = `
                 SELECT userId, status, miniCodeStatus, BGMStatus, alarmStatus, NameStatus, loginType
                 FROM userTB
                 WHERE userId = ?;
                 `;
...
async function checkMaxUserId(connection) {
  const maxUserIdQuery = `
    ...
    limit 1;
  `;
  const [userInfoRow] = await connection.query(maxUserIdQuery);
  return userInfoRow;
}
...
async function insertAppleUserInfo(connection, insertUserInfoParams) {
  const insertUserInfoQuery = `
        INSERT INTO userTB(email,password, nickname,sub, loginType)
        VALUES (?, ?, ?, ?,?);
    `;
  const insertUserInfoRow = await connection.query(
      insertUserInfoQuery,
...
async function insertAppleToken(connection, userId,token, refreshToken) {
  const insertTokenQuery = `
                INSERT INTO tokenTB(userId, jwt, refreshToken)
                VALUES(${userId}, '${token}', '${refreshToken}');
                `;
  const insertTokenRow = await connection.query(insertTokenQuery, userId, token, refreshToken );
...
async function updateAppleUserToken(connection,userId, token, refreshToken) {
  const updateUserTokenQuery = `
  UPDATE tokenTB 
  SET refreshToken = '${refreshToken}',
      jwt = '${token}'
  WHERE userId = ${userId};`;
  const updateUserRow = await connection.query(updateUserTokenQuery, [userId, token, refreshToken]);
  return updateUserRow[0];
}


async function insertUserColor(connection) {
  const UserColorQuery = `
    Insert Into myColorTB (color, colorName,userId) (select color, colorName,userId from basicColorTB) ;
  `;
  const updateUserRow = await connection.query(UserColorQuery);
  return updateUserRow;
}

async function updateIniUserColor(connection,userId) {
  const UserColorQuery = `
    UPDATE myColorTB SET myColorTB.userId = ${userId} WHERE myColorTB.userId = 0;
  `;
  const updateUserRow = await connection.query(UserColorQuery, userId);
  return updateUserRow;
}
