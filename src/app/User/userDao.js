...
async function selectUserSub(connection, sub) {
  const selectUserEmailQuery = `
                SELECT jwt,refreshToken, sub, email
                FROM userTB
...
async function insertAppleUserInfo(connection, insertUserInfoParams) {
  const insertUserInfoQuery = `
        INSERT INTO userTB(email, nickname,sub, loginType)
        VALUES (?, ?, ?,1);
    `;
  const insertUserInfoRow = await connection.query(
      insertUserInfoQuery,
...
async function insertAppleToken(connection, userId,accessToken, refreshToken) {
  const insertTokenQuery = `
                INSERT INTO tokenTB(userId, jwt, refreshToken)
                VALUES(${userId}, '${accessToken}', '${refreshToken}');
                `;
  const insertTokenRow = await connection.query(insertTokenQuery, userId, accessToken, refreshToken );
...
async function updateAppleUserToken(connection,userId, refreshToken) {
  const updateUserTokenQuery = `
  UPDATE tokenTB 
  SET refreshToken = '${refreshToken}'
  WHERE userId = ${userId};`;
  const updateUserRow = await connection.query(updateUserTokenQuery, [userId, refreshToken]);
  return updateUserRow[0];
}
