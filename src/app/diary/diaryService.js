...
// 다이어리 생성
exports.createDiary = async function(userId, content, recordContent, color, colorName, date) {
    try {
        const insertDiaryInfoParams = [userId, content, recordContent, color, colorName, date];
...
exports.editDiary = async function (diaryId, content, recordContent,color,colorName) {
    try {
        ...
        connection.release();
        console.log('수정 성공 : ',updateDiaryResult[0].info)
