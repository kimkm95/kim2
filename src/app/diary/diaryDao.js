...
// 최신 등록순 정렬
async function selectUserHistoryByTime(connection, userId) {
    const userHistoryQuery = `
        select diaryId,color,
               case
                   when userTB.NameStatus = 'N'
...
async function selectDiaryDate(connection, userId, page) {
    const selectDiaryDateQuery = `
        select userTB.userId,diaryId,color,date,content,recordContent,
               case
                   when userTB.NameStatus = 'N'
                       then null
                   else colorName
        ...
        where userTB.userId = ? AND
                diaryTB.date like concat('%', DATE_FORMAT(DATE_ADD(NOW(), INTERVAL ${page} DAY), '%Y-%m-%d'), '%');
     `;
    const [userHistoryRow] = await connection.query(selectDiaryDateQuery, userId, page);
    return userHistoryRow;
}
...
// diaryId 에 맞는 다이어리 수정
async function updateDiary(connection, diaryId, content, recordContent,color,colorName) {
    const updateDiaryQuery = `
        UPDATE diaryTB SET content = IF('${content}' = 'null',null,'${content}'),                
                           recordContent = IF('${recordContent}' = 'null',null,'${recordContent}'),
                           color = '${color}',
                           colorName = '${colorName}'
...
async function insertDiaryInfo(connection, insertDiaryInfoParams) {
    const insertDiaryInfoQuery = `
        INSERT INTO diaryTB(userId, content, recordContent, color, colorName, date)
        VALUES (?, ?, ?, ?, ?, ?);
    `;
...
async function calendarCheck(connection) {
    const checkArticleQuery = `
        SELECT TIMESTAMPDIFF(MONTH, CURDATE(), '2021-01-01') as min,
               ABS(TIMESTAMPDIFF(MONTH, '2030-12-31', CURDATE())) as max;
    `;
...
module.exports = {
    ...
    withdrawSelectImg,
    calendarCheck,
    selectCheckMaxMyColorId,
    selectUserHistoryByTime
