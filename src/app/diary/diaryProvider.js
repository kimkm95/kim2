...
// 유저 전체 다이어리 기록 조회,, 날짜 우선    diaryId,color,colorName,date
...
// 유저 전체 다이어리 기록 조회,, 최신 등록순 기준
exports.selectUserHistoryByTime = async function (userId) {
    const connection = await pool.getConnection(async (conn) => conn);
    const userHistoryCheckResult = await diaryDao.selectUserHistoryByTime(connection, userId);
    connection.release();
...
exports.diaryDateCheck = async function (userId, page) {
    const connection = await pool.getConnection(async (conn) => conn);
    const diaryDateCheckResult = await diaryDao.selectDiaryDate(connection,userId, page);
    connection.release();
...
exports.calendarCheck = async function () {
    const connection = await pool.getConnection(async (conn) => conn);
    const calendarCheckResult = await diaryDao.calendarCheck(connection);
    connection.release();
...
exports.withdrawSelectRecordContent = async function (diaryId) {
    ...
    console.log('RecordContent 삭제 성공')
    connection.release();
