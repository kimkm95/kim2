...
exports.postMyColor = async function(req, res) {
    ...
    */
    ...
    if(status === 'N'){
        ...
        if(!regNum.test(myColorId)) {
            return res.send(errResponse(baseResponse.SIGNUP_MYCOLORID_ERROR_TYPE));
        }

        // myColorId가 내 컬러인지 확인
        const MyColorIdResult = await diaryProvider.myColorIdCheckByMyColorId(myColorId);
        if (MyColorIdResult[0].userId != userId) {
    ...
    if (!myColorId){
        ...
        if(myColorIdRows.length !== 0){
            ...
            if (!color) {
                return res.send(errResponse(baseResponse.DIARY_COLOR_EMPTY));
            }
            if (!colorName) {
                return res.send(errResponse(baseResponse.DIARY_COLORNAME_EMPTY));
            }
            ...
            if (colorName != null && colorName.length > 100) {
                return res.send(errResponse(baseResponse.AUTH_EMAIL_LENGTH));
            }
        ...
        // myColorId가 내 컬러인지 확인
        const MyColorIdResult = await diaryProvider.myColorIdCheckByMyColorId(myColorId);
        console.log(MyColorIdResult[0].userId)
        if (MyColorIdResult[0].userId != userId) {
            return res.send(errResponse(baseResponse.SIGNUP_MYCOLORID_UNMATCHED));
        }

        if (!color) {
            color = MyColorIdResult[0].color
        }
        if (!colorName) {
            colorName = MyColorIdResult[0].colorName
        }
...
exports.getTargetMonthDiary = async function (req, res) {
    ...
    var curr = new Date();	// 현재 날짜 및 시간
    console.log("현재 : ", curr);
    const utc = curr.getTime() + (curr.getTimezoneOffset() * 60 * 1000);
    const KR_TIME_DIFF = 9 * 60 * 60 * 1000;
    const kr_curr = new Date(utc + KR_TIME_DIFF);
    console.log("한국 : " , kr_curr)

    kr_curr.setHours(0, 0, 0, 0);

    var now = moment(kr_curr).format('yyyy-MM')
    var momentDate = moment(date).format('yyyy-MM')

    var strtDate = now;
    var endDate = momentDate;

    var strtYear = parseInt(strtDate.substring(0,4));
    var strtMonth = parseInt(strtDate.substring(5,7));
...
/*
    API No. 16
    API Name : 통합 다이어리 수정 API --> 삭제 시 value 값으로 iplagkeas //  -- 전체 삭제 후, 붙이기 반복 작업
    [PATCH] /app/diarys/final-editing
*/



exports.patchFinalDiary = async function(req, res) {
    // Path Variable : articleId
    const userId = req.verifiedToken.userId;
    ...
    /*
        Body : userId, content, recordContent,color,colorName,date, isManyImg
    */
    var { date, content, recordContent, color, colorName, diaryImgUrl} = req.body;

    if (!date) {
        return res.send(response(baseResponse.DIARY_DATE_EMPTY));
    }
    if(!regDate.test(date)) {
        return res.send(errResponse(baseResponse.SIGNUP_DATE_ERROR_TYPE));
    }

    // 날짜 값을 입력했는데 다이어리가 없으면 등록 있으면 수정으로 넘어간다. 등록 / 수정 API
    if (date) {
        ...
        var now = moment(kr_curr)
        var momentDate = moment(date)
        var page = momentDate.clone().diff(now, 'days')

        const checkDiary = await diaryProvider.diaryDateCheck(userId, page);
        console.log(checkDiary[0])
        console.log('now : ',now)
        console.log('input date : ',date)
        console.log('page number : ' ,page);
        // 다이어리가 이미 존재함. 이경우 바로 수정으로 넘어감
        if (checkDiary.length != 0) {
            console.log('다이어리 존재 -> 수정 시작')
            // 다이어리는 존재하는데, 이미지 값은 수정하지 않을 경우
            if (!diaryImgUrl) {

                if (checkDiary[0].userId != userId) {
                    return res.send(errResponse(baseResponse.USER_ID_DIARYID_NOT_MATCH));
                }
                const diaryId = checkDiary[0].diaryId
                ...
                if (!content) {
                    if(checkDiary[0].content) {
                        content = checkDiary[0].content
                ...
                if (!recordContent) {
                    if(checkDiary[0].recordContent) {
                        recordContent = checkDiary[0].recordContent
                        console.log('before recordContent : ', recordContent)
                    } else {recordContent = null}
                }

                console.log('recordContent : ',recordContent)

                if (!color) {
                    color = checkDiary[0].color
                }
                if (!colorName) {
                    colorName = checkDiary[0].colorName
                }
                ...
                console.log('diary 이미지 수정 시작')
                const insertImgResult = [];
                const diaryId = checkDiary[0].diaryId
                console.log(diaryId)


                // 기존에 있던 다이어리 삭제 후 재 삽입
                // 이미지 삭제 api
                const diaryImgRows = await diaryProvider.retrieveDiaryImg(diaryId);

                for (let i = 0; i < diaryImgRows.length; i++) {

                    diaryImgId = diaryImgRows[i].diaryImgId
                    const diaryImgWithdrawResult = await diaryProvider.withdrawSelectImg(diaryImgId);
                }
        ...
        if (checkDiary.length === 0) {
            ...
            if (diaryImgUrl) {
                ...
                const DiaryResult = await diaryService.createDiary(userId, content, recordContent, color, colorName, date);
                const checkSelectedDiary = await diaryProvider.userHistoryCheck(userId);
                const diaryId = checkSelectedDiary[0].diaryId
...
/*
    API No. 17
    API Name : 다이어리 최종 등록 API
    [POST] /app/diarys/final-posting
*/

exports.postFinalDiary = async function(req, res) {
    // Path Variable : articleId
    const userId = req.verifiedToken.userId;
    ...
    if (date) {
        ...
        if (checkDiary.length === 0) {
            ...
            if (diaryImgUrl) {
                ...
                const DiaryResult = await diaryService.createDiary(userId, content, recordContent, color, colorName, date);
                const checkSelectedDiary = await diaryProvider.selectUserHistoryByTime(userId);
                console.log('방금 등록한 다이어리 : ', checkSelectedDiary[0])
                console.log('방금 등록한 다이어리 아이디 : ',checkSelectedDiary[0].diaryId)
                const diaryId = checkSelectedDiary[0].diaryId
...
exports.calendarCheck = async function(req, res) {
    // Path Variable : articleId
    const userId = req.verifiedToken.userId;


    const calendarCheck = await diaryProvider.calendarCheck();
    console.log(calendarCheck)
    // 다이어리 없음
    return res.send(response(baseResponse.SUCCESS,calendarCheck));
};


/**
 * API No. 13
 * API Name : 다이어리 색상 아카이브 API
 * [GET] /app/diarys/color/history
 */
exports.getMonthColorHistory = async function (req, res) {
