module.exports = function(app){
    ...
    app.patch('/app/diarys/withdrawal',jwtMiddleware, diary.withdrawDiary );

    // API No. 28 캘린더 범위 확인 API
    app.get('/app/diarys/calender/limit',jwtMiddleware, diary.calendarCheck );

    // API No. 29 다이어리 통합 수정 API -- 전체 삭제 후, 붙이기 반복 작업
    app.patch('/app/diarys/final-editing',jwtMiddleware, diary.patchFinalDiary);

    // API No. 30 다이어리 최종 등록 API -- 전체 삭제 후, 붙이기 반복 작업
    app.post('/app/diarys/final-posting',jwtMiddleware, diary.postFinalDiary);

    // API No. 31 다이어리 색상 아카이브 API
    app.get('/app/diarys/color/history',jwtMiddleware, diary.getMonthColorHistory);
};
